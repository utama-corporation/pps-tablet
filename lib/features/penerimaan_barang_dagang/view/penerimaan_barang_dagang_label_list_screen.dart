// lib/features/penerimaan_barang_dagang/view/penerimaan_barang_dagang_label_list_screen.dart
...
class PenerimaanBarangDagangLabelListScreen extends StatefulWidget {
  final String noPenerimaan;

  const PenerimaanBarangDagangLabelListScreen({
    super.key,
    required this.noPenerimaan,
  });

  @override
  State<PenerimaanBarangDagangLabelListScreen> createState() =>
      _PenerimaanBarangDagangLabelListScreenState();
}

class _PenerimaanBarangDagangLabelListScreenState
    extends State<PenerimaanBarangDagangLabelListScreen> {
  late final PenerimaanBarangDagangRepository _repo;
  late Future<PenerimaanBarangDagangDetail> _future;
  ...
  void _enterSelectionMode(String noBarangDagang) {
    setState(() {
      _selectionMode = true;
      _selectedCodes
        ..clear()
  ...
  void _toggleSelection(String noBarangDagang) {
    setState(() {
      if (_selectedCodes.contains(noBarangDagang)) {
        _selectedCodes.remove(noBarangDagang);
        if (_selectedCodes.isEmpty) _selectionMode = false;
      } else {
        _selectedCodes.add(noBarangDagang);
      }
    });
  }
  ...
  Future<void> _bulkPrint() async {
    ...
    final pdfUrls = codes
        .map((c) => Uri.parse(ApiConstants.barangDagangLabelPdf(c)))
        .toList();
  ...
  Future<void> _openAddLabelDialog() async {
    final added = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      builder: (_) => PenerimaanBarangDagangItemFormDialog(
  ...
  Future<void> _openDetailDialog(PenerimaanBarangDagangItem item) async {
    await showDialog<void>(
      ...
      builder: (_) => ProductionOutputDetailDialog(
        labelCode: item.noBarangDagang,
        namaJenis: item.namaBarang,
        printCount: item.hasBeenPrinted,
        accentColor: _kAccent,
        pdfUrl: ApiConstants.barangDagangLabelPdf(item.noBarangDagang),
        feature: 'barang_dagang',
        markAsPrinted: () async {
          final count = await _repo.markItemPrinted(item.noBarangDagang);
          if (mounted) _reload();
          return count;
        },
  ...
  Future<void> _deleteItem(PenerimaanBarangDagangItem item) async {
    final confirmed = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      builder: (_) => ConfirmDialog(
        title: 'Hapus Barang?',
        message:
            'Yakin ingin menghapus ${item.namaBarang} (${item.noBarangDagang})?',
        confirmLabel: 'Hapus',
        confirmIcon: Icons.delete_outline,
      ),
    );
    ...
    try {
      await _repo.deleteItem(item.noBarangDagang);
      if (!mounted) return;
      _reload();
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _buildHeader(PenerimaanBarangDagang header) {
    return Container(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: _selectionMode
          ? null
          : FutureBuilder<PenerimaanBarangDagangDetail>(
              future: _future,
      ...
      body: FutureBuilder<PenerimaanBarangDagangDetail>(
        future: _future,
        builder: (context, snapshot) {
  ...
  Widget _buildItemTile(PenerimaanBarangDagangItem item) {
    final isSelected = _selectedCodes.contains(item.noBarangDagang);
    return Container(
      decoration: BoxDecoration(
      ...
      child: Stack(
        children: [
          InkWell(
            ...
            onTap: () {
              if (_selectionMode) {
                _toggleSelection(item.noBarangDagang);
              } else {
                _openDetailDialog(item);
              }
            },
            onLongPress: () {
              if (!_selectionMode) _enterSelectionMode(item.noBarangDagang);
            },
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
              child: Column(
